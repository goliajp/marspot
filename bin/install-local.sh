...
if (( RUNNING )); then
  ...
  # update). Sessions survive via L3 reattach + bytelog replay —
...
#
# Started two ways.  With a pid argument it was started by the shell
# that is exiting, and waits for exactly that process; with none it was
# started by the LaunchAgent at login, and waits for any marspot-shell.
# The second is only a backstop, and on its own it never worked: the
# app is what the user opens at the start of the day, so launchd's copy
# found it running, waited its half hour and gave up — the bundle stood
# on 0.7.137 from 2026-09-07 to 09-18 with eight installs in between,
# and every cold launch went back to it.
#
# Overridable so the guards below can be exercised against a sandbox
...
OPEN="${MARSPOT_LANDER_OPEN:-/usr/bin/open}"
LOG="$STAGE/land.log"
exec >>"$LOG" 2>&1
...
# Wait for the app to be gone.  Half an hour either way, after which
# the stage stays armed rather than landing under a running process.
i=0
if [ -n "$WAIT_PID" ]; then
  echo "waiting for pid $WAIT_PID (started by the exiting shell)"
  while [ $i -lt 900 ]; do
    kill -0 "$WAIT_PID" 2>/dev/null || break
    sleep 2; i=$((i+1))
  done
  if kill -0 "$WAIT_PID" 2>/dev/null; then
    echo "pid $WAIT_PID still alive after $((i*2))s — leaving the stage armed"
    exit 0
  fi
else
  while [ $i -lt 900 ]; do
...
# Quitting and reopening is one gesture, and the reopened app is on the
# old binary from the bundle this is about to rewrite underneath it —
# which is the AMFI kill this whole mechanism exists to avoid.  Stay
# armed and take the next quit.
if ps auxww 2>/dev/null | grep -F "$MACOS/marspot-shell" | grep -vq grep; then
  echo "the app is running from the bundle again — leaving the stage armed"
  exit 0
fi
for b in marspot-shell marspot-core marspot-session; do
...
disarm
# Only the LaunchAgent's copy reopens the app.  With a pid argument the
# user quit a moment ago and meant it; the binary that lands here is
# what their next launch gets.
[ -n "$WAIT_PID" ] || { "$OPEN" -n "$APP" && echo "relaunched"; }
LANDER
