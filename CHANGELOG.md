...
后台的自行增长 —— 空闲时不该有任何东西在跑。手写版本当年有 8 秒节流,
搬成脚本时丢了。
...
**codex 里被换行切开的 URL 现在能整条识别。** codex 的续行一律顶格（第 0 列），而链接扫描把
顶格的接缝当成最弱的合并信号，URL 一律在那里截断。实测 pane 435（73 列）的一段 401 报错里，
两条 URL 各被切了一次：

- `https://` 在行尾，下一行顶格接 `platform.openai.com/...` —— 截断后只剩 `https://`，不成
  其为 URL，整条不显示链接；
- `https://chatgpt.com/backend-` 在行尾，下一行顶格接 `api/codex/responses` —— 截断后
  **恰好是一个成立的 URL**，于是被链接，点开去的不是屏幕上那条。

顶格接缝默认仍然截断（顶格的正文跟换行长得一模一样，而 URL 没有 `stat` 之类的东西可以仲裁）。
新增一条例外：上一行还剩着空位、而下一行的第一个词放不进那块空位时，这次换行是渲染器被迫做的，
URL 穿过去。这是渲染器自己做换行时的判据，从屏幕上读回来即可，不需要假设 TUI 的内容宽度在哪 ——
这个文件里几乎每一处几何猜测都被现场报告逼着放宽过一次。行排满时不算证据：一行正好在边上结束，
可能是换行也可能是恰好排下，屏幕上分不出来，这时仍然截断。

测试三条：pane 435 的原始行要接上；没被迫的（下一行的词本来放得下）不许接，否则会凭空造出一条
屏幕上没有的链接；行排满时不许接 —— 这条是例外的第一版写错的地方（没剩空位时，任何词看起来都
「放不下」），`marspot-term` 那边的 `zero_indent_does_not_glue_urls` 守的是同一条线。去掉例外后
第一条报红，红的样子跟现场一致。

### 0.12.207

**输入法组字时，底下的字不再透上来。** 预输入本来就画了一块不透明底色，但底色走 BG pass、
字形走 FG pass，而 FG pass 在后面 —— 所以底色只盖住了格子的背景，格子里原有的字随后又被画回
组字文字上面，两层字叠在一起。用中文输入法在 claudecode 的输入框里打字时，它的
placeholder 就是这样糊在拼音上的。

搜索浮层当年踩的是同一个坑，修法也是同一个（F1++ 的 mask）：组字覆盖哪些格子现在由一个纯函数
`preedit_placements` 统一回答，格子循环据此跳过这些格子的字形，绘制也读同一份落位，两边不可能
不一致。落位规则不变：按 UAX #29 字素簇算宽度、到行尾整簇换行、到最后一行为止。

测试两条：落位本身（宽字符、换行、行尾、末行截断），以及「被盖住的格子不发字形」—— 后者去掉
mask 会从 6 个字形变成 9 个，验过能红。
...
带上 L3 0.11.92 的接缝修复（折行接缝不再抢在更长的、存在的终点前面）。扫描在 L2 的
渲染与命中判定里跑，只 bump session 屏幕不会变。改动本身见 L3 0.11.92。
...
上一版那道闸是空的。它在「有帧到达」时就放行，而 `sync_selection_tape` 每次
pump 都跑一遍 —— 一窗十几个 pane 都在出字节，滚轮送出去一两毫秒内就有帧，
程序还根本没重画，闸就又开了。改成**只有读出非零位移**（这一步确实落到画面
上了）才放行；真的没挪动画面时由 120ms 兜底管。

第二个错在判据本身：`matched * 2 >= overlap` 拿重叠区的**总行数**当分母，可
空行永远不可能「对上」。程序的屏幕在消息之间大片是空的，于是一次读得出来的
滚动会输给一个它永远过不了的算术。改成拿**本来就能对上的行数**（旧帧里落在
重叠区内的非空行）当分母。

断带日志现在带上 shift / matched / could / rows，再断就不用猜了。

### 0.12.198

「稍微快一丁点整个选择区就没了」的根因：转发是开环的。0.12.197 限制的是
**单次事件**的幅度，可惯性滚动一秒送几十个事件，而折叠一帧才一次 —— 几步
落在同一次折叠之间，合起来的距离把重叠吃光，读不出来就断带。按事件设上限
治不了按帧发生的问题。

改成反馈驱动：磁带在跟的时候，下一步要等上一步被某一帧读到才发（另有 120ms
兜底，免得一步没挪动画面就把 pane 卡死）。滚轮和「拖到边缘自动滚」两条路
都装了同一道闸 —— 后者尤其需要，鼠标移动事件一秒上百个。

再加两点韧性：偶发读不出的一帧不再直接断带，连续三帧读不出才断，其间拿
**最后一帧读得懂的**当参照（没有同步输出括号的程序会把画到一半的屏幕发出来，
半屏看起来就像不相干的画面）；以及空行不再覆盖磁带里已有的文字 —— 半屏那些
还没画到的行是空的，会在正要复制的那几行上打洞。

### 0.12.197

alt 屏程序（claudecode）的 pane 不往 marspot 的历史里存任何一行 —— 按 VT
规范 alt 屏就是不喂 scrollback —— 而且它滚动的方式是整屏重画，不是叫终端
滚。所以按老办法（相对活底的绝对行号）锚定的选区，会一直解析到同样的屏幕
行上，文字却在底下移动：框钉在玻璃上，复制出来的不是选中的东西。

能救回来的原因是那次重画**是一次干净的整屏平移**。在活的 claudecode pane
上注入滚轮报文、读发布出来的帧量到：3 格滚轮把画面挪了正好 2 行，63 行里
54 行与前一帧逐行相同；10 格挪 16 行，还能对上的 47 行里对上了 39 行；同样
格数反向滚回去，画面逐字节回到起点。

于是选区在这类 pane 上改由一条磁带承载：每帧读出位移，文字按**虚拟行号**
存下来，选区两端记的也是虚拟行号。起点因此永远钉在它的文字上，滚出屏幕也
还在磁带里，复制直接从磁带取 —— 跨屏选择和跨屏复制这才成立。

配套三件：拖拽中的滚轮改为交给程序（我们没有视口可动，磁带负责跟随）；跟踪
期间单次滚动幅度限制在 1/8 屏，让相邻两帧一定有足够重叠（实测一次跳 40 行
只剩 9 行能对上，什么也读不出来）；真读不出来时磁带自己断掉并清掉选区，
而不是留一个会骗人的框。

### 0.12.196

滚轮落在有活选区的 pane 上时，记一行状态：选区属于哪个 pane、滚轮送给
哪个 pane、是否在拖拽中、是否 TUI 鼠标跟踪、是否 alt 屏、我们自己有多少
历史。一次一变化，不是一次一格 —— 惯性滚动一次就是几百格。

加它是因为「选择框不会变动，内容滚动就选了不同的文字」这个现象，三条分支
按代码读都产生不出来：alt 屏 pane 在拖拽路径上一格也滚不动（`scroll_own_view`
的上限就是 0），松手后的路径又会直接清掉选区。真正走的那条只能读出来。
...
- **「磁盘 scrollback 只值 ±4%」——作废。** 那是用 `MARSPOT_DISK_SCROLLBACK=0` 量的,
  ...
  又一次「装置的失效长得跟数据一样」,而且我踩了两遍。项目说明里同一句过期描述一并
  更正。
...
"每帧热路径零分配"这条禁止的形状。但**看代码得出的嫌疑不算数**,给它
...
**动画必须会结束**,这是硬约束:空闲时 CPU 约为 0,不许有动画定时器。所以它不是定时器:值是时钟的纯函数,只在本来就要出帧的时候采样;
...
**DA2 的回复分支是死代码，两周前修过的同一个坑。** `csi_dispatch` 里带 intermediate 的序列必须在
开头那个早返回里答完；写在下面的 `b'c' if intermediates == b">"` 永远匹配不到。2026-09-07 修
XTQVERSION 时就发现过这件事，还在早返回处留了注释写明「下面写的任何 `CSI <int> … <final>` 都是
死的」—— 但 DA2 本人就在下面，没跟着搬。实测：`CSI > c` 返回零字节，而它头顶的注释在描述一个它
并没有发出去的回复。

这类 bug 静默：代码读起来是对的，注释描述的也是对的，没有任何东西会红。所以这次连**不答**的探测
一起做成断言表（`tests/device_queries.rs` 的 `probe_replies`，25 行）。沉默也是行为，将来哪一行要
改，改的就是那一行。

顺带把三个该答没答的补上：

- **DECID（`ESC Z`）** —— DA1 的前身，老 curses 还在发，xterm 用 DA1 的串答它，我们照做。
- **DECXCPR（`CSI ? 6 n`）** —— CPR 的 DEC 私有版。用这个形式问而得不到回答的程序，跟用
  `CSI 6 n` 问而得不到回答的程序等一样久。
- **DECRQM（`CSI ? Ps $ p`）** —— 问某个模式开没开。值得答是因为我们有话可说：确认不了 ?2026 的
  程序就不会用同步输出，而同步输出 marspot 是实现了的，不答等于白造一个功能。不实现的模式报 0
  （规范的「没听说过」），程序照走它的 fallback。两个没有开关的模式说实话：?7 自动换行报 3（永久
  开），?6 原点模式报 4（永久关）—— 比装作不认识对程序更有用。

DECRQM 不走 `record_response`：启动时连问好几个模式是正确客户端的正常行为，一个对正确行为报警的
量具是坏量具。
...
**scrollback 磁盘记录就是 `Cell` 的内存本身**：一行一次拷贝，不再逐格编码。

逐格编码（13 字节/格，每个颜色一次 match）是生产解析路径上唯一有明确价格的一项。先做
消融定价：同一份代码只把编码换成整行拷贝（多写 54% 的字节也算在内），mini 交替 A/B
parse-file +13%～+21%，值得做。实现后同机交替 6 轮（load ≈ 6）：

| 语料 | 改造前 | 改造后 | |
|---|---|---|---|
| ascii | 125.8 | 164.8 MB/s | +31.0%（区间不重叠） |
| cjk | 124.3 | 137.5 MB/s | +10.6% |
| emoji | 98.4 | 106.9 MB/s | +8.6%（区间不重叠） |
| mixed | 105.6 | 114.8 MB/s | +8.7%（区间不重叠） |

**用户已有的历史不重写。** 每条记录本来就带 `rec_len` 和 `cols`，每格宽度 =
`(rec_len-3)/cols`，13 和 20 只在 `cols=0` 时重合（那时没有格子）。打开 v2 文件只把头部
就地改 8 个字节（版本 3、cell_abi 20，并 fsync），之后在 13 字节的旧记录后面追加 20 字节
的新记录，读取按每条记录自己的宽度解码。两处解码（mmap 与 pread）合成一个函数，宽度两个
都对不上的记录拒绝，不猜。

真实数据核对：14 个 pane 的现有 `scrollback.bin`（全是 v2）各拷一份，改造前后的代码
逐行逐格导出（字符 + 全部属性），**逐字节相同**。其中有内容的 8 个 pane 共 15,852 行、
637,242 格；另外 6 个快照时 `.idx` 还没刷出，两边都读 0 行，没验证到。兼容测试另做了
变异验证：关掉 v2 解码分支，两个测试都变红。

回滚方向不覆盖：v3 之前的构建打开 v3 文件会隔离（改名 `.corrupt-<ts>`，不删除）。

新工具 `examples/scrollback_dump.rs`：只用新旧构建都有的 API 导出每一格，专门用来
对比两个构建读同一份历史。
...
候选终点：折行的接缝（`…/rfcs`，目录存在）和 `。`（`…/handoff.md`，文件也存在）。
接缝原来**排在所有标点之前**先试，于是短的那个赢了 —— 可这个函数其余地方的规矩
都是最长优先：文件和它上面那层目录都在的时候，这一行说的是文件。

接缝和标点切点合成一趟、统一按从长到短试。接缝原本要解决的情况（在空格处折行、
两个 token 被拼在一起）不受影响：那种时候接缝右边没有存在的更长终点，它照样被选中。

14 个 pane 的 bytelog 冻结后重放（文件系统答案固定在同一张表里），链接集合
6497 → 6504，减 3 增 10，逐条回原始行核过，全是同一类：原来停在接缝处的目录，
现在延伸到这一行真正写出来的更长终点 —— `path-integrity/` ⏎ `baseline.json'))`
成了完整的 `baseline.json`，`webadmin/` 延伸到下面的三个 `.tsx`，
`Path('…/insight/apps` ⏎ `/consumer/android/app…` 延伸到截断符 `…` 之前。
...
冒号从「token 内部」的字符集里去掉。程序写出来的冒号几乎全是分隔符
（`wrote:path`、`Note:path`、`写好了:path`）；真正属于路径的那个冒号在
span 内部，仍由行号后缀规则裁决，`src/main.rs:42` 不受影响，URL 更早就被
整体匹配掉了。

红-绿都在真数据上：该 pane 的 bytelog 重放，这个 token 在 122 帧里出现过，
改之前只有 74 帧认出链接，改之后 122 帧全认。
...
- **每行一次 `Vec` 分配。** `push_into_ring` 走 `pad_or_clip`,那个函数返回一个新
  ...
  进 ring。热路径的要求是**零分配**,而一次 bulk `cat` 从这里推过去
...
- **有界,所以不会变成内存泄漏**。通道容量固定;磁盘落后时生产者阻塞在 `send`,背压
  沿着 pty 队列传回子进程 —— 写得比磁盘快的 pane 会变慢,但不会变胖
  (「不能越跑越慢」)。
...
上限是为了不让缓冲无限长(每个队列都要有界),不是为了替一次重画拿主意 ——
...
两条自保:攒够 512 KB 自动放弃 hold(pane 里显然有活物,而每个队列都要
有界);以及**用户按键一旦到达 L3 就立刻释放** —— 键能走到这里说明
