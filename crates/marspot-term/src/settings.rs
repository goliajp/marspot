...
pub struct Settings {
    ...
    ///
    /// OFF, and the reason is the cost, which turned out to be much
    /// larger than "someone cats an HTML file": a terminal is where
    /// people TALK about markup.  Switched on globally it ate the tags
    /// out of the conversation that was specifying the feature —
    /// including the user's own words as they came back on screen
    /// (2026-09-06).  Whose output should be treated as markup is not
    /// a property of the terminal; it belongs to the pane, and only
    /// the plugin driving a program knows the program does not render
    /// its own HTML.
