...
//! (U+FE0F), which the parser handles separately.
//!
//! Source: https://www.unicode.org/Public/UCD/latest/ucd/emoji/emoji-data.txt
//! Version: emoji-data.txt for UTS #51 v17.0 (2025-07-25)
//!
//! Regeneration: see bin/regen-emoji-presentation.sh.

/// Sorted, non-overlapping inclusive ranges `(start, end)` of
/// codepoints with `Emoji_Presentation=Yes`.  Binary search via
/// [`has_emoji_presentation`] is the only intended access path.
...
/// Single-pass binary search over `EMOJI_PRESENTATION_RANGES`.
...
mod tests {
    ...
    fn known_presentation_emoji_match_spec() {
        // User-reported via 2026-06-15 emoji rendering complaint.
        assert!(has_emoji_presentation(0x2705), "✅");
        assert!(has_emoji_presentation(0x274C), "❌");
        assert!(has_emoji_presentation(0x2B50), "⭐");
        assert!(has_emoji_presentation(0x1F33F), "🌿");
        // Text-default per the spec — should NOT match.
        assert!(!has_emoji_presentation(0x26A0), "⚠ defaults to text");
        assert!(!has_emoji_presentation(0x261D), "☝ defaults to text");
        // Plain ASCII never matches.
        assert!(!has_emoji_presentation(b"A"[0] as u32));
        assert!(!has_emoji_presentation(0x4E2D), "中 (CJK Unified Ideograph)");
    }
    #[test]
    fn ranges_are_sorted_and_disjoint() {
        ...
        for (i, &(s, e)) in EMOJI_PRESENTATION_RANGES.iter().enumerate() {
            ...
            if i > 0 {
                assert!(prev_end < s, "range[{}] overlaps/adjoins predecessor", i);
            }
            prev_end = e;
        }
    }
